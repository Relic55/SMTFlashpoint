 ...
 * 
 * @author Erik Paluka, Zach Cook, Sören Totzauer
 * @version 1.0
 */
public class Zone extends PGraphics3DDelegate implements PConstants, KeyListener {
	...
	/** The x,y coordinates of a zone (relatively to its parent?). Use (get|set)(X|Y) for value retrival.
	 * Get- and set- Methods will yield coordinates relatively to the topleft corner (0,0) */
	protected int x, y;
	/** Height and Width of a zone. Use (get|set)(Width|Height) for value retrival */
	...
	private boolean moving_enabled = true;
	...
	/* TODO implement quickhull algorithm for polygon bounding? 
	 * Alternativ: V-HACD algorithm ?*/
	
//	public boolean contains(Zone containedZone) {
//		return false;
//	}
	
	/**
	 * Detects if the Zone collides with the supplied collidingZone
	 * by checking if the bounding boxes of the Zones collide, using
	 * the Separating Axis Theorem algorithm.
	 * 
	 * Currently this only works for Zone objects having a rectangular
	 * bounding box. Note that the SAT algorithm furthermore only works for
	 * convex shapes.
	 * 
	 * @param collidingZone - The Zone the collision is checked for
	 * @return true if both zones collide, else false
	 */
	public boolean collidesWith(Zone collidingZone) {
		...
		vertexList_this.addAll(this.getBoundingBox());
		vertexList_coll.addAll(collidingZone.getBoundingBox());
		
		axisList = this.getNormals();
		axisList.addAll(collidingZone.getNormals());
		
		// For every combination of Vertex and Axis do
		
		for(PVector axis : axisList) {
			PVector projection_this = projectOnto(vertexList_this, axis);
			PVector projection_coll = projectOnto(vertexList_coll, axis);
			...
			// If we reach this point, then there is no overlap in projections
			// so at this point we can leave the algorithm and state
	...
	/**
	 * Projects an ArrayList of vertexes onto an axis. The resulting
	 * projection is a vector line segment parallel to the axis.
	 * 
	 * @param vertices	-	A List of vertices from a shape to project
	 * @param axis		-	The axis to project the vertices onto
	 * @return			-	A PVector object depicting the projection
	 */
	private PVector projectOnto(ArrayList<PVector> vertices, PVector axis) {
	...
	/**
	 * This method returns the normals belonging to the edges of the bounding box
	 * of a Zone.
	 * 
	 * @param nodes - The nodes of the bounding box of the Shape
	 * @return an ArrayList of PVector objects representing the normals for the bounding box
	 */
	public ArrayList<PVector> getNormals() {
		ArrayList<PVector> result = new ArrayList<PVector>();
		ArrayList<PVector> nodes = this.getBoundingBox();
	...
	/**
	 * This method will construct a BoundingBox around the Zone depending on its top left corner
	 * - which is specified by the getX() and getY() values of the Zone -, its width and height.
	 * Therefore a Zone which has points outside of this bounding box will yield invalid results.
	 * However this method factors in how the Zone is currently rotated.
	 * @return An ArrayList<PVector> containing four vertices (the corners) of the bounding box.
	 */
	public ArrayList<PVector> getBoundingBox() {
		
		// Using A,B,C,D as denominators for corners.
		//
		// A-----B   topLeftVertex = A = [ zone.getX() , zone.getY() ]
		// |     |   topRightVertex = B = [ zone.getX() + zone.getWidth() , zone.getY() ]
		// |     |   bottomRightVertex = C = [ zone.getX() + zone.getWidth() , zone.getY() + zone.getHeight() ]
		// D-----C   bottomLeftVertex = D = [ zone.getX() , zone.getY() + zone.getY() + zone.getHeight() ]
		...
		PVector origin = new PVector(this.getX(), this.getY());									// Origin of our Card; topleft corner, dependent on global coordinates
		PVector topLeftVertex = new PVector(0,0);														// The UL corner in local coordinates
		
		PVector topRightVertex = new PVector(x * (float) Math.cos(theta) - topLeftVertex.x * (float) Math.sin(theta)	// The UR corner in local coordinates, rotated by theta
								, topLeftVertex.x * (float) Math.cos(theta) + x * (float) Math.sin(theta)
							   );
		
		PVector bottomRightVertex = new PVector(x * (float) Math.cos(theta) - y * (float) Math.sin(theta)	// The LR corner in local coordinates, rotated by theta
				, y * (float) Math.cos(theta) + x * (float) Math.sin(theta)
			   );
		
		PVector bottomLeftVertex = new PVector(topLeftVertex.x * (float) Math.cos(theta) - y * (float) Math.sin(theta)	// The LL corner in local coordinates, rotated by theta
				, y * (float) Math.cos(theta) + topLeftVertex.x * (float) Math.sin(theta)
				   );
		...
		topLeftVertex.add(origin);
		topRightVertex.add(origin);
		bottomRightVertex.add(origin);
		bottomLeftVertex.add(origin);
		
		result.add(topLeftVertex);
		result.add(topRightVertex);
		result.add(bottomRightVertex);
		result.add(bottomLeftVertex);
...
//			Integer.MIN_VALUE, Integer.MAX_VALUE
			-2*this.getWidth(), Integer.MAX_VALUE,
			-2*this.getHeight(), Integer.MAX_VALUE);
	}
	...
	public void drag(
		...
		try {
			first = touches[0];
		} catch(ArrayIndexOutOfBoundsException e) { // This shouldn't throw anymore, hopefully.
			e.printStackTrace();
			return;
		}
		...
		//clamp right delta
		if( delta.x > 0)
			if( ! drag_right || limit_right <= (this.getX() + this.getWidth()))
				delta.x = 0;
			else {
				delta.x = Math.min( delta.x, (limit_right - (this.getX() + this.getWidth())));
			}
		//clamp left delta
		if( delta.x < 0)
			if( ! drag_left || limit_left >= this.getX()) {
				delta.x = 0;
			}
			else {
				delta.x = Math.max( delta.x, (limit_left - this.getX()));
			}
		//clamp down delta
		if( delta.y > 0)
			if( ! drag_down || limit_down <= this.getY() + this.getHeight()) {
				delta.y = 0;
			}
			else {
				delta.y = Math.min( delta.y, (limit_down - (this.getY() + this.getHeight())));
			}
		//clamp up delta
		if( delta.y < 0)
			if( ! drag_up || limit_up >= this.getY()) {
				delta.y = 0;
			}
			else {
				delta.y = Math.max( delta.y, (limit_up - this.getY()));
			}
	...
	public void hSwipe(int leftLimit, int rightLimit){
		...
		drag(true, false, leftLimit, rightLimit, 0, Integer.MAX_VALUE);
	}
...
//		drag(false, true, Integer.MIN_VALUE, Integer.MAX_VALUE, upLimit, downLimit);
		drag(false, true, 0, Integer.MAX_VALUE, upLimit, downLimit);
	}
...
//			ArrayList<PVector> boxParent = parent.getBoundingBox();
//			ArrayList<PVector> boxThisZone = this.getBoundingBox();
//			
//			for(PVector testPoint : boxThisZone) {
//				for(int i=0, j = boxParent.size()-1; i<boxParent.size(); j=i++) {
//				
//					 if ((boxParent.get(i).y > testPoint.y) != (boxParent.get(j).y > testPoint.y) &&
//					            (testPoint.x < (boxParent.get(j).x - boxParent.get(i).x) * (testPoint.y - boxParent.get(i).y) / (boxParent.get(j).y-boxParent.get(i).y) + boxParent.get(i).x)) {
//					          drag(true, true, true, true);
//					 }
//				}
//			}
			
			drag( true, true,true,true,
				parent.getX(), parent.getX() + parent.getWidth(),
				parent.getY(), parent.getY() + parent.getHeight());
		}
	...
	public void assignTouch(Touch... touches){
		assignTouch( Arrays.asList( touches));
	}
	 ...
	 */
	public void assignTouch( Iterable<? extends Touch> touches){
	 ...
	 */
	public void unassignTouch( Touch touch){
		unassignTouch( touch.sessionID);
	}
	 ...
	 */
	public void unassignTouch( long sessionID){
	...
	public boolean add( Zone zone){
		if( zone != null){
			...
			"Tried to add null to this zone: "+this.toString());
		return false;
	}
	...
	/**
	 * Enable or disable moving. If disabled rst() and drag() won't have any effect on that zone.
	 * @param enabled
	 */
	...
	/**
	 * Check whether a zone may move or not
	 * @return the state of the movement-enabling flag
	 */
	public boolean isMoveable() {
		return this.moving_enabled;
	}
	 ...
	 */
	public boolean isCapturingTouches(){
	...
	public void printAncestry() {
		...
		while(parent != null) {
			System.out.println(parent + " is Parent of " + me);
			me = parent;
