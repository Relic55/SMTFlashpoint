...
public class TextZone extends Zone {
	...
	private class WordZone extends Zone {
		...
		public WordZone( WordZone original) {
			super( original.getX(), original.getY(), original.width, original.height);
	...
	public TextZone(TextZone original) {
		super(original.name, original.getX(), original.getY(), original.width, original.height);
	...
	public TextZone(int x, int y, int width, int height, String inputText, boolean selectable,
			boolean blur, boolean keysReceivedFromApplet) {
		this(null, x, y, width, height, inputText, selectable, blur, keysReceivedFromApplet, 16);
	}
	...
	public TextZone(String name, int x, int y, int width, int height, String inputText,
			boolean selectable, boolean blur, boolean keysReceivedFromApplet, float fontSize) {
		this(name, x, y, width, height, inputText, selectable, blur, keysReceivedFromApplet, fontSize, null);
	}
	 ...
	 * @param blur  - boolean:
	 * @param keysReceivedFromApplet  - boolean:
	 ...
	 */
	public TextZone(String name, int x, int y, int width, int height, String inputText,
			boolean selectable, boolean blur, boolean keysReceivedFromApplet, float fontSize, PFont font) {
		super(name, x, y, width, height);
		this.currentWordZone = new WordZone(5, 5, 0, 20);
		this.keysFromApplet = keysReceivedFromApplet;
		if (keysReceivedFromApplet) {
			applet.registerMethod("keyEvent", this);
		}
	...
	public TextZone(int x, int y, int width, int height, boolean keysreceivedFromApplet) {
		this(null, x, y, width, height, keysreceivedFromApplet);
	}
	...
	public TextZone(String name, int x, int y, int width, int height, boolean keysreceivedFromApplet) {
		this(name, x, y, width, height, null, false, false, keysreceivedFromApplet, 16f);
	}
	...
	public void addChar(char c) {
		if (c == ' ') {
			this.currentWordZone = new WordZone(currentWordZone.getX() + currentWordZone.width,
					currentWordZone.getY(), 0, (int) (fontSize * (20.0 / 16.0)));
			currentWordZone.word += " ";
		}
		else if (c == '\t') {
			this.currentWordZone = new WordZone(currentWordZone.getX() + currentWordZone.width,
			...
			this.currentWordZone = new WordZone(5, currentWordZone.getY()
					+ (int) (fontSize * (20.0 / 16.0)), 0, (int) (fontSize * (20.0 / 16.0)));
		}
		else {
			if (currentWordZone.word.trim().equals("")) {
				currentWordZone = new WordZone(currentWordZone.getX() + currentWordZone.width,
						currentWordZone.getY(), 0, (int) (fontSize * (20.0 / 16.0)));
			}
			if (currentWordZone.getX() + currentWordZone.width + fontSize > width
					&& currentWordZone.getX() != 0) {
//				this.currentWordZone.setData(0, currentWordZone.getY()
//						+ (int) (fontSize * (20.0 / 16.0)), 0, (int) (fontSize * (20.0 / 16.0)));
				this.currentWordZone.setSize(0, currentWordZone.getY() + (int) (fontSize * (20.0 / 16.0)) );
				this.currentWordZone.setLocation(0, currentWordZone.getY() + (int) (fontSize * (20.0 / 16.0)));
			}
			if (currentWordZone.getX() + currentWordZone.width + fontSize <= width) {
