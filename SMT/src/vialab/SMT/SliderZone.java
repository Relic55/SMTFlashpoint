...
public class SliderZone extends Zone {
	...
	private int colorKnob = 0xFFBBBBBB;
	private int colorSlider = 0xFFFFFFFF;
	...
	public SliderZone(String name, int x, int y, int width, int height, int currentValue,
		...
		if(maxValueLabel.equals(""))
			minValueLabel = Integer.toString(minValue);
		if(maxValueLabel.equals(""))
			minValueLabel = Integer.toString(maxValue);
	}
	
	public void setDrawCurrentValue(boolean flag) {
		drawCurrentValue=flag;
	}
	
	public void setMinLabel(String txt) {
		minValueLabel = txt;
	}
	
	public void setMaxLabel(String txt) {
		maxValueLabel = txt;
	}
...
//		text(minValue, width/10, height*9/10);
		...
		text(minValueLabel, width/10, height*9/10);
		text(maxValueLabel, width*9/10, height*9/10);
...
//		text(maxValue, width*9/10, height*9/10);
	...
	protected void drawTicks(){
		...
		if(majorTickSpacing <= 0 && minorTickSpacing <= 0) return;
		
		for (int i = minValue; i <= maxValue; i++){
			if (majorTickSpacing > 0 && i % majorTickSpacing == 0){
				// draw major tick
				line(((i / (float) maxValue) * width * (float) 8 / 10) + (width / 10), height * 3 / 8,
						((i / (float) maxValue) * width * (float) 8 / 10) + (width / 10),
						height * 5 / 8);
			}
			else if (minorTickSpacing > 0 && i % minorTickSpacing == 0){
				// draw minor tick
				line(((i / (float) maxValue) * width * (float) 8 / 10) + (width / 10), height * 3 / 8,
						((i / (float) maxValue) * width * (float) 8 / 10) + (width / 10),
						height / 2);
